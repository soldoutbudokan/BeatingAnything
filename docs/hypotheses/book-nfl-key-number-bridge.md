...
**Completed result:** [The fixed backtest](../../reports/nfl-key-number-backtest-2026-09-13.md) selected zero games: no eligible fresh half-point price pair crossed three or seven. ROI is undefined, not zero. This archive cannot evaluate this specific opportunity, so park it without adding integer lines, other key margins or wider entry windows. The [unchanged declaration copy](../nfl-key-number-declaration-2026-09-13.md) preserves the original pre-comparison card bytes and hash; this paragraph is a later result annotation.
